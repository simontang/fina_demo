...
// ============================================================
// Tool executives（从 tools.ts 原封不动迁移）
// ============================================================
...
export const sapB1Plugin: Plugin = {
  meta: {
    ...
    description: "SAP Business One Service Layer OData API 连接器，支持 BP/物料/订单/库存四大领域的查询与操作",
    version: "1.0.0",
    configSchema: {
      type: "object",
      properties: {
  ...
  connection: {
    ...
    discover: async () =>
      API_LIST.map((e) => ({
        ...
        description: e.description,
        kind: e.kind,
        primaryKey: e.primaryKey || undefined,
      })),
    resourceLabel: "Entity / Function Import",
  },
  ...
  middleware: (rawConfig) => {
    ...
    return createMiddleware({
      ...
      tools: [
        tool(
          (input) => sapApiSearchExecutor(input as Parameters<typeof sapApiSearchExecutor>[0], config),
          {
            name: "sap_api_search",
            description: sapApiSearchDescription,
            schema: z.object({
              query: z
                .string()
                .optional()
...
PluginRegistry.register(sapB1Plugin);
