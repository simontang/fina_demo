...
import com.fina.b1s.dto.SemanticQueryRequest;
...
/**
 * Generates SQL Server SQL from a semantic query for a single metric.
 *
 * Generation steps:
 *   1. Resolve dim_ids in group_by → actual SQL Server field names (via supported_dimensions)
...
public class SemanticQueryBuilderImpl implements SemanticQueryBuilder {
    ...
    public BuildResult build(String metricName,
        ...
        for (SemanticQueryRequest.OrderByItem ob : orderBys) {
            ...
            // Use the alias name (quoted) so SQL Server can resolve computed columns
    ...
    /**
     * Resolve a filter dimension string to the actual SQL Server field name.
